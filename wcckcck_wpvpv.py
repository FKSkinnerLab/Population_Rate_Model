...
wcckcck = np.linspace(-0.35, -0.05, grid_size)
wpvpv = np.linspace(-0.09, -0.01, grid_size)
p_space = np.meshgrid(wcckcck, wpvpv)
...
for i in tqdm(range(len(wpvpv))):
    for j in range(len(wcckcck)):
        new_cell._set_connections()
        new_cell.wcckcck = wcckcck[j] # x value
        new_cell.wpvpv = wpvpv[i] # y value
